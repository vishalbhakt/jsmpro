...
export default function AdminEnquiries() {
  ...
  const fetchEnquiries = async () => {
    setLoading(true);
    try {
      const res = await cmsAPI.inquiries.list();
      setData(res.data);
    } catch (err) {
      console.error("Failed to fetch inquiries", err);
      addToast("Failed to sync inquiry stream", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResolve = async (id: number) => {
    try {
      await cmsAPI.inquiries.update(id, { status: "closed" });
      addToast("Inquiry marked as closed/resolved.");
      fetchEnquiries();
    } catch {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await cmsAPI.inquiries.delete(id);
      ...
      <div className="space-y-10 pb-20">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-slate-500 font-medium mt-1">Institutional record of public inquiries, admission requests, and system messages.</p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              >
                 <div className="flex justify-between items-start relative z-10">
                    ...
                    <div className="flex flex-col items-end gap-2">
                       ...
                       {item.status === 'closed' ? (
                          <div className="px-3 py-1 bg-emerald-500 text-white rounded-lg text-[8px] font-black uppercase tracking-[0.2em]">Closed</div>
                       ) : (
                          <div className="px-3 py-1 bg-amber-500 text-white rounded-lg text-[8px] font-black uppercase tracking-[0.2em] animate-pulse">{item.status}</div>
                       )}
                    </div>
                 </div>

                 <div className="relative z-10 space-y-2">
                    <h3 className="text-3xl font-black text-[#001f3f]">{item.student_name}</h3>
                    <div className="flex items-center gap-3 text-[#d4af37] font-black uppercase tracking-[0.15em] text-[10px]">
                       <ArrowUpRight className="w-3 h-3" />
                       Class: {item.preferred_class || "General Inquiry"}
                    </div>
                 </div>

                 <div className="relative z-10 space-y-4 bg-slate-50 p-6 rounded-[2rem] border border-[#001f3f]/5">
                    <div className="text-xs font-bold text-[#001f3f]/70">Guardian: {item.guardian_name || "-"}</div>
            ...
            {!data.length && (
              <div className="col-span-full py-40 text-center bg-white rounded-[4rem] border-2 border-dashed border-[#001f3f]/5 flex flex-col items-center">
                 ...
                 <p className="text-slate-300 font-black uppercase tracking-[0.3em] text-xs italic">Registry Empty</p>
              </div>
            )}
          </div>
        )}
